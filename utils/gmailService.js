...
class GmailService {
    ...
    async sendBreachAlert(to, cowId, alarmType, location, timestamp) {
        ...
        const message = `
            ...
            <p><a href="${process.env.APP_URL || 'http://localhost:3000'}/tracking.html" style="background-color: #dc2626; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; display: inline-block; margin: 20px 0;">View Real-time Tracking</a></p>
        `;
    ...
    async sendSystemAlert(to, alertType, message, details = {}) {
        ...
        const emailMessage = `
            ...
            <p><a href="${process.env.APP_URL || 'http://localhost:3000'}/dashboard.html" style="background-color: #dc2626; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; display: inline-block; margin: 20px 0;">Go to Dashboard</a></p>
        `;
